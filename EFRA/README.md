...
2. The user must then execute the bash script ```prepare_input.sh```, which will take in input the files generated at step 1 and will output a folder named ```openb_pod_list_default``` containing the YAMLs with the specifications of the EFRA datacenter and workload.
