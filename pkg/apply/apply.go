...
func (applier *Applier) Run() (err error) {
	...
	fmt.Printf("DEBUG FRA, Applier Run() => number of applications in the appList: %d\n", len(applier.appList))
	var appResource simulator.ResourceTypes
	for _, app := range applier.appList {		
		// process separately chart and other file
		if app.Chart {
			fmt.Printf("DEBUG FRA => application files are located into a Helm chart. Converting...\n")
			...
			fmt.Printf("DEBUG FRA => application files are located into a YAML. Loading and converting its content into Kubernetes objects...\n")
	...
	if applier.cluster.KubeConfig != "" {
		fmt.Printf("DEBUG FRA, Applier Run() => KubeConfig found!\n")
		...
		fmt.Printf("DEBUG FRA, Applier Run() => Simulated cluster config found!\n")
		if clusterResource, err = simulator.CreateClusterResourceFromClusterConfig(applier.cluster.CustomCluster); err != nil {
			return err
		}
	}
	fmt.Printf("DEBUG FRA, Applier Run() => Simulated cluster config: %+v\n", clusterResource)
	...
	if applier.interactive {
		fmt.Printf("DEBUG FRA, Applier Run() => interactive mode is enabled.\n")
